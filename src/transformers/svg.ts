// this file is slightly adapted from xstate-viz pathUtils and Edges.tsx
...
export const pointsToPath = (
  ...
  const path = [
    ["M", startPoint],
    ...points.slice(1, -1).map((point) => ["L", point]),
  ];
  ...
  path.push(["L", finalPoint]);

  return path as SvgPath;
};
...
export const sigmoid = (start: Point, end: Point): string => {
  ...
  var bX = start.x + xDir * xDiff * 0.05;
  var bY = start.y;

  var cX = start.x + xDiff * 0.33;
  var cY = start.y;
  var dX = end.x - xDiff * 0.33;
  var dY = end.y;
  var eX = end.x - xDir * xDiff * 0.05;
  var eY = end.y;
